...
fn emit_rust_source(schemas: &[SchemaIR]) -> Result<String, CodegenError> {
    ...
    out.push_str("// AUTO-GENERATED by kcl-rust-codegen — DO NOT EDIT BY HAND.\n");
    out.push_str(
        "// Regenerate by re-running the codegen tool against the source KCL.\n\n",
    );
    out.push_str("#![allow(clippy::derive_partial_eq_without_eq)]\n");
    out.push_str("#![allow(non_snake_case)]\n\n");

    // Phase 4 step 4 lands the actual emission. Step 2 just produces
    // a placeholder banner + a structured "schemas seen" comment so
    // the crate compiles and round-trips end-to-end through the
    // pipeline. Subsequent steps fill in the real emission body.
    for schema in schemas {
        out.push_str(&format!("// schema: {}\n", schema.name));
        for field in &schema.fields {
            out.push_str(&format!(
                "//   field: {} : {:?} (optional={}, default={})\n",
                field.name, field.kind, field.optional, field.has_default
            ));
        }
        out.push('\n');
    }
...
mod tests {
    ...
    #[test]
    fn generate_to_string_emits_banner_and_schema_summary() {
        let src = "schema Trivial:\n    placeholder: int = 0\n";
        let out = generate_to_string(src).expect("generate should succeed");
        assert!(out.contains("AUTO-GENERATED by kcl-rust-codegen"));
        assert!(out.contains("schema: Trivial"));
        assert!(out.contains("field: placeholder"));
    }
