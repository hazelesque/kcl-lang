use kcl_runtime::{
    ...
    schema_config_meta, schema_runtime_type, separate_kv, split_type_union, try_coerce_mokkan_inet,
    val_plan,
};
                ...
                if let Some(index) = s.schemas.borrow().get(&schema_type_name) {
                        ...
                        && value.is_config()
                    {
                        ...
                        for (key, _) in &config.values {
                            let no_such_attr = !SchemaEvalContext::has_attr(s, &caller.ctx, key)
                                && !key.starts_with('_');
